...
async function fetchFilesQuery(
  userId: string,
  groupId: string | null,
  isAdmin: boolean,
): Promise<FileRecord[]> {
  ...
  // Admin: fetch all files visible via RLS (policy must allow admin reads)
  if (isAdmin) {
    const { data, error } = await db
      .from('files')
      .select(SELECT)
...
export function useFiles(groupId: string | null = null) {
  ...
  const key = ['files', user?.id ?? null, groupId, isAdmin] as const;
  ...
  const {
    ...
    queryKey: key,
    queryFn: () => fetchFilesQuery(user!.id, groupId, isAdmin),
    enabled: !!user,
    staleTime: 3 * 60 * 1000,
  });

  const error =
    queryError instanceof Error ? queryError.message : queryError ? 'Failed to fetch files' : null;
