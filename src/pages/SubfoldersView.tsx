...
const s = {
  ...
  backBtn:
    'flex items-center gap-1.5 text-xs text-slate-500 hover:text-slate-800 transition-colors border-0 bg-transparent cursor-pointer p-0',
  sidebarLabel:
    'px-4 pt-5 pb-1.5 text-[10px] font-semibold uppercase tracking-widest text-slate-400',
  ...
  // cards
  detailsCard:
    'w-[340px] shrink-0 bg-white rounded-xl border border-slate-200 flex flex-col self-start',
  ...
  filesTh:
    'text-left px-5 py-2.5 text-[10px] font-semibold uppercase tracking-widest text-slate-400',
  ...
  downloadBtn:
    'opacity-0 group-hover:opacity-100 transition-opacity flex items-center gap-1 text-xs text-slate-400 hover:text-indigo-600 cursor-pointer border-0 bg-transparent p-0',

  // empty / loading
  emptyWrap: 'flex flex-col items-center justify-center py-16 gap-3',
  emptyIcon:
    'w-10 h-10 rounded-xl bg-slate-50 border border-slate-200 flex items-center justify-center text-slate-300',
  ...
  return (
    ...
    >
      {hov && !renaming && canEdit && (
        <div
          style={{ position: 'absolute', top: -8, right: -8, zIndex: 10, display: 'flex', gap: 3 }}
        >
          <button
            title="Rename"
            onClick={(e) => {
      ...
      >
        ...
        <div
          style={{
            ...
            alignItems: 'center',
            justifyContent: 'center',
            boxShadow: hov ? `0 8px 24px ${body}66` : `0 2px 8px ${body}44`,
        ...
        >
          ...
          />
          <span style={{ fontSize: 24, filter: 'drop-shadow(0 1px 3px rgba(0,0,0,0.25))' }}>
            {node.icon ?? '📁'}
          </span>
          {(node.file_count ?? 0) > 0 && (
            <div
...
export default function SubfoldersView() {
  ...
  const cohortMismatch =
    !dismissedWarning &&
    !!(profile?.cohort && currentGroup?.name && profile.cohort !== currentGroup.name);
  ...
  function jumpTo(index: number) {
    setFolderStack((s) => (index === -1 ? [] : s.slice(0, index + 1)));
  }
  function toggleAll() {
  ...
  async function handleExport() {
    ...
    try {
      ...
      const response = await fetch(`${import.meta.env.VITE_SUPABASE_URL}/functions/v1/export-zip`, {
        method: 'POST',
        headers: {
  ...
  const pagedRoots = roots.slice(
    (safeProjPage - 1) * PROJ_PAGE_SIZE,
    safeProjPage * PROJ_PAGE_SIZE,
  );
      ...
      {cohortMismatch && (
        <div className="flex items-start gap-3 border-b border-amber-200 bg-amber-50 px-5 py-3 text-sm text-amber-800">
          ...
          <span className="flex-1 leading-relaxed">
            <strong>View &amp; download only</strong> — your cohort (
            <strong>{profile?.cohort}</strong>) doesn't match this group (
            <strong>{currentGroup?.name}</strong>).{' '}
            <button
              type="button"
              onClick={() => navigate('/settings')}
              className="cursor-pointer border-0 bg-transparent p-0 text-sm font-medium underline hover:text-amber-900"
            >
          ...
          <div className={s.topbar}>
            ...
            <nav className={s.breadcrumb} aria-label="Folder path">
              ...
              <button
                type="button"
                onClick={() => jumpTo(-1)}
                className="cursor-pointer border-0 bg-transparent p-0 text-sm text-slate-500 hover:text-slate-800"
              >
                  ...
                  {i === folderStack.length - 1 ? (
                    <>
                      <button
                        type="button"
                        onClick={() => jumpTo(i - 1)}
                        className="flex cursor-pointer items-center border-0 bg-transparent p-0 text-slate-400 hover:text-slate-700"
                        title="Go back"
                      >
                        <ArrowLeft size={13} />
                      </button>
            ...
            <div className={s.topActions}>
              {canEdit && isLeafFolder && (
                <button
                  ...
                  onClick={() =>
                    navigate('/upload', {
                      state: { folderId: activeFolder!.id, groupId: resolvedGroupId },
                    })
                  }
                  className="flex h-8 cursor-pointer items-center gap-1.5 rounded-md border-0 bg-violet-600 px-3 text-xs font-medium text-white transition-colors hover:bg-violet-700"
                >
              ...
              {isLeafFolder && (
                <button
                  ...
                  className={[
                    s.exportBtn,
                    exporting || selectedIds.size === 0 ? 'cursor-not-allowed opacity-40' : '',
                  ].join(' ')}
                >
                  {exporting ? (
                    ...
                    {projTotalPages > 1 && (
                      <div className="mt-6 flex items-center justify-between border-t border-slate-100 pt-4">
                        <span className="text-[12px] text-slate-400">
                          {(safeProjPage - 1) * PROJ_PAGE_SIZE + 1}–
                          {Math.min(safeProjPage * PROJ_PAGE_SIZE, roots.length)} of {roots.length}{' '}
                          projects
                        </span>
                        <div className="flex items-center gap-1">
                          <button
                            ...
                            className="flex h-8 w-8 cursor-pointer items-center justify-center rounded-lg border border-slate-200 bg-white text-sm text-slate-500 transition-colors hover:border-[#054159] hover:bg-[#054159] hover:text-white disabled:cursor-not-allowed disabled:opacity-30"
                          >
                            ‹
                          </button>
            ...
            <div className="flex flex-col gap-5 p-6">
              <div>
                <div className="mb-3 flex items-center justify-between">
                  ...
                  <span className="text-[11px] text-slate-400">
                    {currentChildren.length} folder{currentChildren.length !== 1 ? 's' : ''}
                  </span>
                </div>
                <div className="rounded-xl border border-slate-200 bg-white p-6">
            ...
            <div className={s.content}>
              ...
              <div className={s.detailsCard}>
                <div className={s.cardHeader}>
                  ...
                  <button
                    type="button"
                    className="cursor-pointer border-0 bg-transparent p-0 text-slate-300 hover:text-slate-500"
                  >
              ...
              <div className={s.filesCard}>
                <div className={s.cardHeader}>
                  ...
                  <span className={s.cardMeta}>
                    {loadingFiles
                      ? 'Loading…'
                      : `${files.length} file${files.length !== 1 ? 's' : ''}${files.length > 0 ? ` · ${formatBytes(totalSize)}` : ''}`}
                  </span>
                </div>
                    ...
                    <tbody className={s.filesTbody}>
                      {files.map((f) => {
                        ...
                        return (
                          <tr
                            key={f.id}
                            className={[s.filesTr, isSelected ? 'bg-indigo-50/50' : ''].join(' ')}
                          >
                            ...
                            <td className={s.filesTd + ' text-slate-400 tabular-nums'}>
                              {f.sizeFormatted}
                            </td>
                            <td className={s.filesTd}>
                              <span
                                className={[
                                  'inline-flex items-center rounded px-2 py-0.5 text-[10px] font-bold tracking-wide text-white uppercase',
                                  badgeCls(ext),
                                ].join(' ')}
                              >
                                {ext?.toUpperCase()}
        ...
        >
          ...
          >
            ...
            <div className="text-center">
              <h2 className="mb-1 text-[15px] font-semibold text-slate-900">No cohort selected</h2>
            ...
            <div className="mt-1 flex gap-3">
              <button
                type="button"
                onClick={() => setShowCohortDialog(false)}
                className="h-10 flex-1 cursor-pointer rounded-xl border-0 bg-red-50 text-sm font-medium text-red-600 hover:bg-red-100"
              >
                Cancel
              </button>
              <button
