...
function FolderTile({ node, canEdit, onOpen, onRename, onDelete }: {
  ...
  return (
    ...
    >
      ...
      {/* Bottom: file count + subfolder count pills */}
    ...
    <div onClick={e => { if (e.target === e.currentTarget) onClose(); }}
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md flex flex-col" onClick={e => e.stopPropagation()}>
        <div className="flex items-center justify-between px-6 pt-6 pb-4 border-b border-slate-100">
          ...
          <button type="button" onClick={onClose} className="w-8 h-8 flex items-center justify-center rounded-lg bg-slate-100 text-slate-500 hover:bg-slate-200 border-0 cursor-pointer">
            <X size={15} />
          </button>
        </div>
...
function FilePreview({ file, ext }: { file: ProjectFile; ext: string }) {
  ...
  const isText   = ['txt','md','csv','json','xml','html','css','js','ts','jsx','tsx',
                    'py','java','c','cpp','rb','go','rs','sh','yaml','yml','toml','ini','log'].includes(ext);
      ...
      try {
        if (isOffice || isVideo || isAudio) {
          const { data, error: e } = await supabase.storage
            .from('filevault').createSignedUrl(storagePath, 3600);
          ...
          const { data, error: e } = await supabase.storage
            .from('filevault').download(storagePath);
          if (e || !data) throw new Error(e?.message ?? 'Failed');
          if (isText) {
  ...
  if (!canPreview) {
    return (
      <div className="flex flex-col items-center justify-center py-14 gap-3">
        <div className="w-14 h-14 rounded-2xl flex items-center justify-center text-[12px] font-bold"
          style={{ background: cfg.bg, color: cfg.color }}>
          {cfg.label.slice(0, 3).toUpperCase()}
        </div>
        <p className="text-[13px] text-slate-500 font-medium">No preview for .{ext} files</p>
  ...
  if (error) {
    return (
      <div className="flex flex-col items-center justify-center py-14 gap-2 text-center px-8">
        ...
        <p className="text-[13px] text-slate-500 font-medium">Preview unavailable</p>
  ...
  if (url && isImage) {
    return <img src={url} alt={file.name} className="max-w-full object-contain p-4" style={{ maxHeight: '70vh' }} />;
  }

  if (url && (isPdf || isOffice)) {
    return <iframe src={url} title={file.name} className="w-full border-none" style={{ height: '72vh' }} />;
  }
  ...
  if (url && isAudio) {
    return (
      <div className="flex flex-col items-center gap-5 py-12 px-8">
        <div className="w-16 h-16 rounded-2xl flex items-center justify-center text-[13px] font-bold"
          style={{ background: cfg.bg, color: cfg.color }}>
          {ext.toUpperCase()}
        </div>
  ...
  if (text !== null) {
    return (
      <pre className="w-full text-left p-5 font-mono text-[12px] leading-relaxed text-slate-700 whitespace-pre-wrap break-words overflow-auto"
    ...
    <>
      ...
      <div className="fixed inset-0 z-[70] flex items-center justify-center p-6 pointer-events-none">
        <div
          ...
          style={{ width: '100%', maxWidth: 860, maxHeight: '92vh', animation: 'previewPop 0.18s ease-out' }}
          onClick={e => e.stopPropagation()}
        >
          <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100 shrink-0">
            ...
            <button onClick={onClose}
              className="w-8 h-8 flex items-center justify-center rounded-lg bg-slate-100 text-slate-500 hover:bg-slate-200 border-0 cursor-pointer shrink-0 ml-3">
              <X size={14} />
            </button>
          </div>
...
function FileDrawer({ file, folderName, groupName, canEdit, onClose, onDeleted }: {
  file: ProjectFile;
  folderName?: string;
  groupName?: string;
  canEdit: boolean;
  ...
  async function handleDownload() {
    ...
    setDownloading(true);
    try { await downloadFile(storagePath, file.name); }
  ...
  return (
    <>
      ...
      <div
        className="fixed right-0 top-0 bottom-0 z-50 w-[360px] bg-white border-l border-slate-200 flex flex-col shadow-2xl"
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100 shrink-0">
          <div className="flex items-center gap-3 min-w-0">
            ...
            <div className="min-w-0">
              ...
              <p className="text-[11px] text-slate-400">{formatBytes(Number(file.size_bytes ?? 0))} · .{ext.toUpperCase()}</p>
            </div>
          </div>
          <button onClick={onClose}
            className="w-7 h-7 flex items-center justify-center rounded-lg bg-slate-100 text-slate-500 hover:bg-slate-200 border-0 cursor-pointer shrink-0 ml-2">
            <X size={13} />
          </button>
        </div>
        ...
        <div className="flex items-center gap-2 px-5 py-3 border-b border-slate-100 shrink-0">
          <button onClick={() => void handleDownload()} disabled={downloading}
            className="flex items-center gap-1.5 h-8 px-3 rounded-lg text-[12px] font-medium text-slate-700 bg-slate-50 border border-slate-200 hover:bg-slate-100 cursor-pointer transition-colors disabled:opacity-50">
            {downloading ? <Loader2 size={12} className="animate-spin" /> : <Download size={12} />}
            {downloading ? 'Downloading…' : 'Download'}
          </button>
          <button onClick={() => setPreviewOpen(true)}
            className="flex items-center gap-1.5 h-8 px-3 rounded-lg text-[12px] font-medium text-violet-600 bg-violet-50 border border-violet-100 hover:bg-violet-100 cursor-pointer transition-colors">
            <Eye size={12} /> Preview
          </button>
          {canEdit && (
            <button onClick={() => void handleDelete()} disabled={deleting}
              className="flex items-center gap-1.5 h-8 px-3 rounded-lg text-[12px] font-medium text-red-600 bg-red-50 border border-red-100 hover:bg-red-100 cursor-pointer transition-colors disabled:opacity-50 ml-auto">
          ...
          <div className="border border-slate-100 rounded-xl px-4 py-3">
            ...
            {uploader ? (
              <div className="flex items-center gap-3">
                <MemberAvatar name={uploader.full_name ?? uploader.email ?? 'User'} avatarUrl={uploader.avatar_url} size={36} />
                <div className="min-w-0">
                  <p className="text-[13px] font-medium text-slate-700 truncate">{uploader.full_name ?? 'Unknown'}</p>
                  {uploader.email && <p className="text-[11px] text-slate-400 truncate">{uploader.email}</p>}
                </div>
              </div>
            ) : (
              <div className="flex items-center gap-3">
...
export default function ProjectDetail() {
  ...
  // ✅ NEW: read router location state so catalog clicks can deep-link into a folder
  ...
  // ✅ NEW: track whether we've already consumed the deep-link state so it only fires once
  const [deepLinkConsumed,   setDeepLinkConsumed]   = useState(false);

  // Reset folder navigation when the group changes
  ...
  // Flat map of all folders keyed by id
  ...
  // ✅ NEW: Auto-drill into the folder passed via router state (from My Catalog click)
  useEffect(() => {
    ...
    // Walk up via parent_id to build the ancestor path from root → target
    ...
    if (path.length > 0) {
      setFolderIdStack(path);
    }
    setDeepLinkConsumed(true);
  }, [location.state, foldersLoading, roots, folderMap, deepLinkConsumed]);

  // Resolve live records from the map so children are always up-to-date
  ...
  const visibleTiles  = filterTileId ? currentTiles.filter(f => f.id === filterTileId) : currentTiles;
  ...
  const { data: members = [], isLoading: membersLoading } = useQuery<SubProjectMember[]>({
    ...
    queryFn: async () => {
      ...
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const folderMemberIds = new Set((fm ?? []).map((m: any) => m.user_id));
      const merged: { id: string; user_id: string; role: string }[] = [
        ...(fm ?? []),
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        ...(gm ?? []).filter((m: any) => !folderMemberIds.has(m.user_id)),
      ];
  ...
  const cohortMismatch      = !dismissedWarning && !!(profile?.cohort && group?.name && profile.cohort !== group.name);
      ...
      <Layout>
        <div className="flex-1 flex items-center justify-center bg-slate-50">
          <div className="text-center">
            <p className="text-sm text-slate-500 mb-3">Project not found.</p>
            <button onClick={() => navigate('/groups')} className="text-sm text-violet-600 hover:underline border-0 bg-transparent cursor-pointer">← Back to cohorts</button>
          </div>
        </div>
      </Layout>
    );
        ...
        {cohortMismatch && (
          <div className="flex-shrink-0 flex items-start gap-3 px-6 py-3 bg-amber-50 border-b border-amber-200 text-sm text-amber-800">
            ...
            <p className="flex-1 leading-relaxed">
              <strong>View &amp; download only</strong> — Your cohort (<strong>{profile?.cohort}</strong>) doesn't match this group (<strong>{group.name}</strong>).{' '}
              <button onClick={() => navigate('/settings')} className="underline text-amber-700 hover:text-amber-900 border-0 bg-transparent cursor-pointer p-0 text-sm">
                Update in Settings → Profile
              </button>
            </p>
            <button onClick={() => setDismissedWarning(true)} className="text-amber-400 hover:text-amber-600 border-0 bg-transparent cursor-pointer p-0 shrink-0">
              <X size={14} />
            </button>
          </div>
        )}
        ...
        <div className="flex-1 overflow-y-auto bg-slate-50">
          ...
          <div className="px-6 py-4 bg-white border-b border-slate-200 flex items-center justify-between">
            <div>
              <nav className="flex items-center gap-1.5 text-[11px] text-slate-400 mb-1">
                ...
                <button onClick={() => jumpTo(-1)}
                  className="hover:text-violet-600 border-0 bg-transparent cursor-pointer p-0 text-[11px] text-slate-600 font-medium">
                  ...
                  <span key={f.id} className="flex items-center gap-1.5">
                    ...
                    {i === folderStack.length - 1 ? (
                      ...
                      <button onClick={() => jumpTo(i)}
                        className="hover:text-violet-600 border-0 bg-transparent cursor-pointer p-0 text-[11px] text-slate-600">
                        {f.name}
                      </button>
                    )}
                  </span>
              ...
              <h1 className="text-[16px] font-semibold text-slate-900 flex items-center gap-2">
                {activeFolder ? (
                  <><span className="text-xl">{activeFolder.icon ?? '📁'}</span>{activeFolder.name}</>
                ) : (
                  <>{group.icon ? <span className="text-xl">{group.icon}</span> : null}{group.name}</>
                )}
              </h1>
            </div>
            ...
            <div className="flex items-center gap-2">
              ...
              {/* New Sub-folder — orange gradient */}
              ...
              {/* Upload File — orange gradient primary */}
              {canEdit && isLeafFolder && (
                <button
                  onClick={() => navigate('/upload', { state: { folderId: activeFolder!.id, groupId } })}
                  className="flex items-center gap-1.5 h-9 px-4 rounded-lg text-xs font-bold text-white border-0 cursor-pointer transition-all hover:-translate-y-px"
                  style={{
                    background: 'linear-gradient(to right, #FF9A00, #FF6B00, #E85500)',
              ...
              {/* Create Project — orange gradient primary */}
              <button
              ...
              <div className="flex gap-5 items-start">
                ...
                <div className="flex-1 min-w-0 flex flex-col gap-4">
                  ...
                  <div className="bg-white rounded-xl border border-slate-200">
                    <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100">
                      ...
                      <button className="text-slate-300 hover:text-slate-500 border-0 bg-transparent cursor-pointer p-0">
                    ...
                    <div className="divide-y divide-slate-50">
                      <div className="flex items-center px-5 py-3.5">
                        ...
                        <span className="text-[13px] text-slate-800 font-medium">{activeFolder.name}</span>
                      </div>
                      <div className="flex items-center px-5 py-3.5">
                        <span className="w-28 text-[13px] text-slate-400">Description</span>
                        <span className="text-[13px] text-slate-800 font-medium">
                          {(activeFolder as any).description ?? <span className="text-slate-300">—</span>}
                        </span>
                      </div>
                      <div className="flex items-center px-5 py-3.5">
                        <span className="w-28 text-[13px] text-slate-400">Cohort</span>
                        <span className="text-[13px] text-slate-800 font-medium">{group.name}</span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="bg-white rounded-xl border border-slate-200">
                    ...
                    {membersLoading ? (
                      ...
                      <div>
                        <div className="grid px-5 py-2 bg-slate-50/60 border-b border-slate-100"
                          ...
                          {members.map(m => (
                            <div key={m.id}
                              ...
                              <div className="flex items-center gap-2.5 min-w-0">
                                <MemberAvatar name={m.full_name ?? m.email ?? 'User'} avatarUrl={m.avatar_url} size={28} />
                                <span className="text-[13px] font-medium text-slate-700 truncate">{m.full_name ?? 'Unknown'}</span>
                              </div>
                ...
                <div className="flex-1 min-w-0 bg-white rounded-xl border border-slate-200 flex flex-col">
                  <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100">
                    ...
                    <div className="flex items-center gap-2">
                      <button className="flex items-center gap-1.5 h-7 px-3 rounded-lg text-[11px] font-medium text-slate-600 bg-slate-50 border border-slate-200 hover:bg-slate-100 cursor-pointer transition-colors">
                        <Upload size={11} className="rotate-180" /> Export
                      </button>
                      <button className="w-6 h-6 flex items-center justify-center rounded text-slate-400 hover:text-slate-600 hover:bg-slate-100 border-0 bg-transparent cursor-pointer">
                  ...
                  {!filesLoading && files.length > 0 && (
                    <div className="grid px-5 py-2 bg-slate-50/60 border-b border-slate-100"
                      ...
                      <span className="text-[10px] font-semibold uppercase tracking-wider text-slate-400">Name</span>
                      <span className="text-[10px] font-semibold uppercase tracking-wider text-slate-400 text-right">Size</span>
                      <span className="text-[10px] font-semibold uppercase tracking-wider text-slate-400 text-right pr-2">Type</span>
                      <span />
                    </div>
                  )}
                  ...
                  {filesLoading ? (
                    ...
                    <div className="py-16 flex flex-col items-center gap-3 text-center px-6">
                      ...
                      <p className="text-[12px] text-slate-400 max-w-[200px]">
                        {canEdit ? 'Upload your first file to this project.' : 'Files will appear here once uploaded.'}
                      </p>
                      {canEdit && (
                        <button onClick={() => navigate('/upload', { state: { folderId: activeFolder.id, groupId } })}
                    ...
                    <div className="divide-y divide-slate-50">
                      {files.map(file => {
                        ...
                        return (
                          <div key={file.id}
                            className="grid items-center px-5 py-3.5 hover:bg-slate-50/60 cursor-pointer group transition-colors"
                            style={{ gridTemplateColumns: '1fr 80px 60px 36px' }}
                            ...
                            <button
                              onClick={e => { e.stopPropagation(); void downloadFile(file.path ?? file.storage_path ?? '', file.name); }}
                              className="w-7 h-7 flex items-center justify-center rounded-md text-slate-300 hover:text-slate-600 hover:bg-slate-100 border-0 bg-transparent cursor-pointer opacity-0 group-hover:opacity-100 shrink-0">
                              <Download size={13} />
                            </button>
                          </div>
                        );
              ...
              <div className="bg-white rounded-xl border border-slate-200">
                <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100">
                  <div>
                    ...
                    <p className="text-[12px] text-slate-400 mt-0.5">
                      {foldersLoading ? 'Loading…' : `${currentTiles.length} folder${currentTiles.length !== 1 ? 's' : ''}`}
                    </p>
                  </div>
                </div>
                <div className="p-6">
                  {foldersLoading ? (
                    ...
                    <div className="flex flex-col items-center py-14 gap-3">
                      ...
                      <p className="text-[12px] text-slate-400">
                        {canEdit ? 'Create your first project above.' : 'Projects will appear here once created.'}
                      </p>
                    </div>
                  ) : (
                    <>
                      ...
                      <div className="flex items-center gap-2 flex-wrap mb-5">
                        <button
                          ...
                          style={filterTileId === null ? { background: 'linear-gradient(to right, #FF9A00, #FF6B00, #E85500)', boxShadow: '0 4px 12px rgba(255,100,0,0.35)' } : {}}
                        >
                          All
                        </button>
                        {currentTiles.map(f => (
                          <button
                            key={f.id}
                            type="button"
                            onClick={() => setFilterTileId(prev => prev === f.id ? null : f.id)}
                            className={[
                              'inline-flex items-center gap-1 h-8 px-4 rounded-full text-sm font-medium transition-all duration-150 cursor-pointer border whitespace-nowrap',
                              filterTileId === f.id
                                ? 'text-white border-transparent shadow-md'
                                : 'bg-white text-slate-600 border-slate-200 hover:border-slate-300 hover:text-slate-900',
                            ].join(' ')}
                            style={filterTileId === f.id ? { background: 'linear-gradient(to right, #FF9A00, #FF6B00, #E85500)', boxShadow: '0 4px 12px rgba(255,100,0,0.35)' } : {}}
                          >
                            <span>{f.icon ?? '📁'}</span>
                            {f.name}
                            {(f.file_count ?? 0) > 0 && (
                              <span className={filterTileId === f.id ? 'opacity-70 text-xs' : 'text-slate-400 text-xs'}>
                                {f.file_count}
                              </span>
                            )}
                          </button>
