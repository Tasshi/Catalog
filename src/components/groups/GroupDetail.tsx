...
const pillActive = 'text-white border-transparent bg-[#1E3A8A] shadow-sm';
const pillInactive =
...
function GroupDetailInner({ group }: GroupDetailProps) {
  ...
  return (
    <div className="flex h-full flex-col">
      {group.description && <p className="mb-4 text-sm text-slate-500">{group.description}</p>}
...
function MoveFolderPicker({
  ...
  return (
    ...
    >
      ...
      >
        <h3 className="mb-1 text-base font-semibold text-slate-800">Move file</h3>
        <p className="mb-4 truncate text-xs text-slate-500">"{file.name}"</p>
        {roots.length === 0 ? (
        ...
        <button
          className="mt-4 h-9 w-full rounded-lg border border-red-200 text-sm text-red-600 transition-colors hover:bg-red-50"
          onClick={onClose}
        >
          Cancel
        </button>
