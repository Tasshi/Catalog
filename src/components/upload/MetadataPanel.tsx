...
//           .from('group_members')
//           .insert({ group_id: groupId, user_id: existing.id, role: 'viewer' });
//         if (memberError) {
...
//       .select('id, email, full_name, phone, role')
//       .eq('group_id', groupId)
//       .then(({ data }: { data: Invitation[] | null }) => {
//         if (!cancelled) {
//           setInvitations((data as Invitation[]) ?? []);
//           setInvLoading(false);
//         }
...
export default function MetadataPanel({ files, onFilesSelected, onSubmit, uploading, progress, currentFolderId = null }: MetadataPanelProps) {
  ...
  return (
    <div className="bg-white border border-[#D4DEE9] rounded-[5px] shadow-[0_1px_2px_rgba(0,0,0,0.04)] p-8 flex flex-col gap-6">
      ...
      {/* ── File picker — always visible so user can select files first ── */}
      <FormField label="Files">
        <FilePicker
          files={files}
          onFilesSelected={onFilesSelected}
          disabled={uploading || uploadBlockedByGroup}
        />
      </FormField>
